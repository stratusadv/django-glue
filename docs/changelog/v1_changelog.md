...
- **Batched, searchable `foreign_key_choices()`**: a relation field (FK or M2M) can opt into seek-paginated choice loading by declaring `foreign_key_choice_config = {field_name: {'search_field': ..., 'batch_size': ...}}` on its `ModelForm`, the same seek (keyset) pagination `Glue.queryset()` already uses (`GlueCollectionCursor`). The field proxy gains `hasMoreChoices`, `loadMoreChoices()`, `searchChoices(query, searchField)`, and `clearSearch()`; `searchChoices()` runs an `icontains` server-side search on the declared field and takes over the field's choices (mirroring `scroll.html`'s `scroll_queryset_filter_field` convention) until `clearSearch()` reverts to the default, unsearched batch. `foreign_key_choices()`'s return shape changed from a bare list to `{results, has_next, seek_key}` for both `FormGlue` and `ModelGlue` (any direct caller needs updating); a field with no `foreign_key_choice_config` entry is unaffected -- it keeps receiving every related row in one response, exactly as before.
