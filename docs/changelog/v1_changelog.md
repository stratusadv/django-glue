...
- **Flexible, searchable `Glue.choices()` sources**: static Django choices remain local native iterables, while queryset sources can declare server-owned `search_fields`, `search_limit`, and rich `fields`. Searchable relations return no unfiltered collection and return at most the configured number of matches per query; deliberately small non-searchable querysets load fully. Form and model registrations can carry configured querysets through `related_field_config[field_name]['choice_queryset']`. The browser exposes `searchChoices(query)` and `clearSearch()`, retains selected rich choices after search clears for both single and multiple relation fields, and ignores stale out-of-order responses.
