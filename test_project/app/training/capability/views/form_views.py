...
@permission_required('training.delete_trainingcapability')
def delete_form_modal_view(request: WSGIRequest, pk: int) -> TemplateResponse:
    ...
    form_action = reverse(
        'training:capability:delete_form_modal',
        kwargs={'pk': pk}
    )
    ...
    def add_activity() -> None:
        capability.add_activity(
            ...
            information=f'{request.user.get_full_name()} deleted a capability on "{capability.training}".'
        )

    fallback = reverse(
        'training:detail',
        kwargs={'pk': capability.training.pk}
    )
...
@permission_required('training.change_trainingcapability')
