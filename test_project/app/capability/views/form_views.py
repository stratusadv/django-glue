...
from test_project.app.capability import forms, models
...
@permission_required('app.delete_appcapability')
def delete_form_modal_view(request: WSGIRequest, pk: int) -> TemplateResponse:
    ...
    form_action = reverse(
        'app:capability:delete_form_modal',
        kwargs={'pk': pk}
    )
    ...
    def add_activity() -> None:
        capability.add_activity(
            ...
            information=f'{request.user.get_full_name()} deleted a capability on "{capability.app}".'
        )

    fallback = reverse(
        'app:detail',
        kwargs={'pk': capability.app.pk}
    )
...
@permission_required('app.change_appcapability')
